...
- NEVER use `var()` inside `className` — use Tailwind semantic classes instead (see `## Critical Rules`).
- NEVER use hex colors in `className` — always use Tailwind color classes.
- Use `cn()` for conditional or merged classes; use plain `className` for static-only styles.
- Use `style` prop only for truly dynamic runtime values (e.g., percentages, progress).
- Reserve `var()` for library props (like Recharts) that can't accept `className`.

## Resources

- [Tailwind CSS 4 Docs](https://tailwindcss.com/docs) — Official documentation.
- [tailwind-merge](https://github.com/dcastil/tailwind-merge) — Conflict-safe class merging.
