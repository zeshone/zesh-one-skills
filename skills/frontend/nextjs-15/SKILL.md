...
3. **DO implement mutations through Server Actions; DO NOT call backend write endpoints directly from client components; WHY: endpoint exposure is reduced and auth/validation stays server-side.**
...
10. **DO reserve `route.ts` handlers for webhooks/third-party callbacks; DO NOT use route handlers as default internal mutation API layer; WHY: Server Actions are primary mutation boundary here.**
...
12. **DO keep client boundaries deep and small; DO NOT turn whole pages/layouts into client components; WHY: broad client boundaries increase bundle size and hydration cost.**
...
- Direct client-side mutation calls to backend APIs.
...
- Duplicating Server Action mutation logic in route handlers.
...
2. Add middleware protection for access and security policy.
3. Implement mutation via Server Action (validate, authorize, mutate, revalidate).
4. Introduce minimal client component only where interaction requires it.
5. Add route-local `loading.tsx`/`error.tsx` for resilience.

Quick reviewer checklist:

- Confirm route exists under `app/` and not `pages/`.
- Confirm mutation path lives in Server Action.
